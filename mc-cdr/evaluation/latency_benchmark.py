...
def _record_latency(
    ...
    tracker.record(
        LatencyRecord(
            ...
            event_timestamp=event_start_us,
            detection_latency_us=detection_time_us - event_start_us,
            response_latency_us=response_time_us - event_start_us,
            provider=provider,
        )
    )
...
def run_latency_benchmark() -> dict:
    ...
    for i in range(500):
        ...
        event_start_us = time.perf_counter() * 1_000_000
        ...
        detection_time_us = time.perf_counter() * 1_000_000
        response_time_us = time.perf_counter() * 1_000_000
        _record_latency(
            tracker,
            normalized.provider,
            str(uuid.uuid4()),
